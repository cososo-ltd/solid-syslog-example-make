...
Every record carries `[meta sequenceId="N"]`. The counter increments once per record *formatted*,
not per record delivered, so a record that never arrives leaves a gap rather than no trace at all.

That earns its place from the next stage on, where a full buffer can start dropping records.

<!-- STAGE-COST:START (generated by scripts/gen-cost-table.py — do not edit by hand) -->

**Cost above baseline: Flash +6,036 B, RAM +432 B.**
