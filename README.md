...
SolidSyslog is in the build: the core library, plus the sources that integrate lwIP in raw mode,
compiled against this device's own `lwipopts.h`. Nothing calls it yet, so the linker strips it all
back out.
